...
- сохранение авторских записей каждого завершённого узла: состояния, свидетельство слоя, индукция, инверсия и самоотчёт о сохранении оси;
...
После проведения формулы доступна кнопка «СОХРАНИТЬ ПРОЙДЕННЫЕ УЗЛЫ JSON». Она скачивает файл с завершёнными узлами текущего цикла; незавершённый черновик в него не входит. В конце цикла остаётся кнопка «СОХРАНИТЬ ЧЕРТЁЖ JSON».

Авторские записи находятся в `authoredSteps`, игровые расчёты — в `simulation`. Статус `progress.status` различает частичный и завершённый цикл. Фактический `q` остаётся `null`: слова игрока и игровые баллы не превращаются в независимое подтверждение возврата. [Контракт экспорта](docs/GAME_TRACE_AUTHOR_RECORDS.md).

Это скачивание файла, не автосохранение и не восстановление сессии. Сохраните нужные узлы до перезагрузки или закрытия вкладки.
