...
export const MODEL_VERSION = "0.1.0-candidate";
...
export function compileTzarLanguage(state, options = {}) {
  ...
  const observedQ = state.observedQ == null ? null : Number(state.observedQ);
  return {
    modelId: MODEL_ID,
    ...
    layers: {
      ...
      publicStatement: `Я различаю наблюдаемое — «${object}» — и его отражение — «${image}». Моё действительное движение сейчас — ${need}. Я сохраняю основание «${invariant}» и совершаю проверяемый ход: ${move}. Ответом станет не обещание, а наблюдаемый отклик: ${feedback}.`,
