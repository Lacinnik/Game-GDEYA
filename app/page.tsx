...
import { captureAuthoredStep, createGameTrace } from "./game-trace.mjs";
...
type LogItem={card:Card;quality:number;az:string;buka:string;tx:string;geometry:string;point:string;deltas:Metrics;source:number;layer:string;authored:ReturnType<typeof captureAuthoredStep>};
type DictationTarget="intent"|"invariant"|"state"|"reflection"|"induction"|"inversion";
type VoiceRecognition={lang:string;interimResults:boolean;maxAlternatives:number;continuous:boolean;start:()=>void;stop?:()=>void;abort?:()=>void;onresult:((event:{resultIndex?:number;results:{[key:number]:{[key:number]:{transcript:string}}}})=>void)|null;onerror:((event:{error?:string})=>void)|null;onend:(()=>void)|null};
...
function coherencePreview(m:Metrics,gradient:number){return clamp(metricAverage(m)-(Math.max(...Object.values(m))-Math.min(...Object.values(m)))*.28-Math.max(0,gradient-m.iy)*.15)}
...
export default function Home(){
 ...
 const [skelluOpen,setSkelluOpen]=useState(false);const [voiceEnabled,setVoiceEnabled]=useState(false);const [voiceReady,setVoiceReady]=useState(false);const [recognitionReady,setRecognitionReady]=useState(false);const [speaking,setSpeaking]=useState(false);const [listening,setListening]=useState(false);const [voiceNotice,setVoiceNotice]=useState("");
 const voicesRef=useRef<SpeechSynthesisVoice[]>([]);const speechRunRef=useRef(0);const lastNarrationRef=useRef("");const recognitionRef=useRef<VoiceRecognition|null>(null);const recognitionRunRef=useRef(0);
 const intentRef=useRef<HTMLTextAreaElement>(null);const invariantRef=useRef<HTMLTextAreaElement>(null);const stateInputRef=useRef<HTMLInputElement>(null);const reflectionRef=useRef<HTMLTextAreaElement>(null);const inductionRef=useRef<HTMLInputElement>(null);const inversionRef=useRef<HTMLInputElement>(null);
 const card=deck[turn];const layer=layers[turn%layers.length];
 ...
 const narration=screen==="intro"?"Я Скеллу. Я могу вести тебя по живому циклу голосом, читать карты и удерживать структуру шага. Открой меня и включи ведение.":screen==="setup"?"Начнём с осевания. Сначала назови живое намерение: что ты проводишь через цикл. Затем назови инвариант: что должно сохраниться при любых изменениях формы.":screen==="end"?`Цикл свёрнут. Итоговая когерентность ${coherencePreview(metrics,gradient)}. Не оценивай себя. Посмотри, где формула проводила ядро, а где поле забирало внимание.`:!card?"Я рядом.":stage==="observe"?`Узел ${turn+1} из ${deck.length}. Плита ${card.plate.toLowerCase()}. Карта: ${card.process.toLowerCase()}. Фаза ${card.roman}. Назови ровно ${card.arabic} ${card.arabic===1?"состояние":card.arabic<5?"состояния":"состояний"}. Не объясняй их, только различи.`:stage==="shape"?`Теперь выбери геометрию узла и тип точки. Геометрия отвечает на вопрос: как устроено поле. Тип точки показывает, что именно происходит с движением.`:stage==="act"?"Собери формулу хода. Аз — твоя внутренняя частота. Бука — внешняя форма. Передача — оператор, который проводит изменение. Затем проверь: сохраняет ли ход ось.":`${lastResult?.message??"Поле ответило."} Проводимость хода ${Math.round((lastResult?.quality??0)*100)} процентов. Посмотри на изменение метрик и переходи к следующему узлу, когда будешь готов.`;
 const dictationTarget:DictationTarget|undefined=screen==="setup"?(!intent.trim()?"intent":!invariant.trim()?"invariant":undefined):screen==="game"&&stage==="observe"&&card&&draft.states.length<card.arabic?"state":screen==="game"&&stage==="shape"?"reflection":screen==="game"&&stage==="act"?(!draft.induction.trim()?"induction":!draft.inversion.trim()?"inversion":undefined):undefined;
 ...
 function cancelSpeech(){speechRunRef.current+=1;window.speechSynthesis?.cancel();setSpeaking(false)}
 function stopListening(notice=false){dictationGuardRef.current.cancel();const recognition=recognitionRef.current;if(!recognition)return;recognitionRunRef.current+=1;recognitionRef.current=null;try{if(recognition.abort)recognition.abort();else recognition.stop?.()}catch{}setListening(false);if(notice)setVoiceNotice("Слушание остановлено.")}
 const speak=useCallback((text:string)=>{if(!voiceReady)return;stopListening();const run=++speechRunRef.current;window.speechSynthesis.cancel();setSpeaking(false);const utterance=new SpeechSynthesisUtterance(text);utterance.lang="ru-RU";utterance.rate=.92;utterance.pitch=.92;const voices=voicesRef.current.length?voicesRef.current:window.speechSynthesis.getVoices();const russian=voices.find(v=>v.lang.toLowerCase().startsWith("ru"));if(russian)utterance.voice=russian;utterance.onstart=()=>{if(speechRunRef.current===run)setSpeaking(true)};utterance.onend=()=>{if(speechRunRef.current===run)setSpeaking(false)};utterance.onerror=()=>{if(speechRunRef.current===run)setSpeaking(false)};lastNarrationRef.current=text;window.speechSynthesis.speak(utterance)},[voiceReady]);
 ...
 function conduct(){if(!card)return;const a=az.find(x=>x.id===draft.az)!,b=buka.find(x=>x.id===draft.buka)!,g=geometries.find(x=>x.id===draft.geometry)!,tx=transmissions.find(x=>x.id===draft.tx)!;const pt=points.find(x=>x.id===draft.point)!;
  ...
  const next:Metrics={alpha:clamp(metrics.alpha+deltas.alpha),iy:clamp(metrics.iy+deltas.iy),cm:clamp(metrics.cm+deltas.cm),q:clamp(metrics.q+deltas.q),t:clamp(metrics.t+deltas.t)};setMetrics(next);setGradient(x=>clamp(x+5+quality*9));const ns=clamp(source+(quality>.72?1:quality<.35?-1:0));setSource(Math.min(11,Math.max(0,ns)));const message=quality>=.78?"Формула проводит намерение и сохраняет ядро.":quality>=.55?"Ход состоялся, но часть энергии ушла на внутреннее сопротивление.":quality>=.35?"Форма движется, однако связь с инвариантом ослабла.":"Поле захватило ход: требуется стабилизация и пересборка.";setLastResult({quality,deltas,message});setLog(v=>[...v,{card,quality,az:a.title,buka:`${b.symbol} ${b.title}`,tx:tx.title,geometry:g.title,point:draft.point,deltas,source:ns,layer:layer.title,authored:captureAuthoredStep(draft)}]);setStage("result");try{navigator.vibrate?.(quality>.65?20:[28,35,28])}catch{}
 }
 ...
 function exportTrace(){const blob=new Blob([JSON.stringify(createGameTrace({intent,invariant,mode,source:sourcePhases[source],metrics,gradient,coherence,log,completed:screen==="end"}),null,2)],{type:"application/json"});const u=URL.createObjectURL(blob),a=document.createElement("a");a.href=u;a.download="subject-core-trace.json";document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(u),1000)}

 return <main className="appShell">
  <aside className="evidenceNotice">Учебная симуляция, не психологическая диагностика. Qsim — игровой балл; фактический Q не наблюдался и остаётся null.</aside>
  ...
  {screen==="end"&&<section className="end screen"><div className="kicker">Цикл свёрнут</div><div className="finalCore" style={{"--score":`${coherence*3.6}deg`} as React.CSSProperties}><div><b>{coherence}</b><span>КОГЕРЕНТНОСТЬ</span></div></div><h2>{coherence>=82?"СУБЪЕКТ ПРОВЁЛ ФОРМУ":coherence>=66?"АРХИТЕКТУРА СОБРАНА":coherence>=48?"ЯДРО СОХРАНЕНО НЕ ПОЛНОСТЬЮ":"ФОРМА ЗАХВАТИЛА ЯДРО"}</h2><p className="endText">{coherence>=82?"Намерение достигло формы, а Источник остался несущим основанием результата.":coherence>=66?"Цикл завершён с сохранённой проводимостью. Несколько связей требуют интеграции.":coherence>=48?"Материализация состоялась ценой части осевой устойчивости. Траектория показывает место сноса.":"Градиент стал сильнее позиции. Архив цикла сохранил точку, из которой возможна новая сборка."}</p><div className="finalFormula"><span>Исходное намерение</span><b>{intent}</b><i>⊕</i><span>Сохранённый инвариант</span><b>{invariant}</b></div><div className="finalStats"><Metric label="α" value={metrics.alpha}/><Metric label="IЯ" value={metrics.iy}/><Metric label="Cₘ" value={metrics.cm}/><Metric label="Qsim" value={metrics.q}/><Metric label="T" value={metrics.t}/></div><div className="dominants"><div><span>Ведущий Аз</span><b>{dominant("az")}</b></div><div><span>Ведущая Бука</span><b>{dominant("buka")}</b></div><div><span>Ведущая Передача</span><b>{dominant("tx")}</b></div></div><button className="primary" onClick={()=>setScreen("setup")}>НОВЫЙ ВИТОК</button><button className="secondary" onClick={exportTrace}>СОХРАНИТЬ ЧЕРТЁЖ JSON</button></section>}

  <div className="skelluDock"><button className={`skelluOrb ${speaking?"speaking":""} ${listening?"listening":""}`} aria-label="Открыть голосового помощника Скеллу" onClick={()=>{if(skelluOpen)closeSkellu();else setSkelluOpen(true)}}><i>◉</i><span>СКЕЛЛУ</span></button>{skelluOpen&&<aside className="skelluPanel" aria-live="polite"><header><div className="skelluAvatar"><i>◉</i></div><div><b>СКЕЛЛУ</b><span>{listening?"слушает":speaking?"говорит":voiceEnabled?"ведёт цикл":"голосовой проводник"}</span></div><button aria-label="Закрыть Скеллу" onClick={closeSkellu}>×</button></header><p>{narration}</p><div className="skelluActions"><button className={voiceEnabled?"active":""} onClick={toggleVoice}>{voiceEnabled?"ВЫКЛЮЧИТЬ ГОЛОС":"ВКЛЮЧИТЬ ВЕДЕНИЕ"}</button><button onClick={()=>speak(narration)} disabled={!voiceReady}>↻ ПОВТОРИТЬ ШАГ</button></div>{dictationLabel&&<button className={`dictation ${listening?"active":""}`} onClick={recognitionReady?listenContext:focusDictationField}>{listening?"■ ОСТАНОВИТЬ СЛУШАНИЕ":recognitionReady?`● ${dictationLabel}`:"⌨ ОТКРЫТЬ ДИКТОВКУ КЛАВИАТУРЫ"}</button>}<small>Распознавание зависит от браузера; офлайн-работа не подтверждена. </small><small>{voiceNotice||(!recognitionReady?"Распознавание Safari недоступно: используй системную диктовку клавиатуры.":"Говори коротко: одна мысль — одно нажатие.")}</small></aside>}</div>
