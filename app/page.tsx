...
type VoiceRecognition={lang:string;interimResults:boolean;maxAlternatives:number;continuous:boolean;start:()=>void;onresult:((event:{results:{[key:number]:{[key:number]:{transcript:string}}}})=>void)|null;onerror:(()=>void)|null;onend:(()=>void)|null};
...
export default function Home(){
 ...
 const dictationLabel=screen==="setup"?(!intent.trim()?"НАЗВАТЬ НАМЕРЕНИЕ":!invariant.trim()?"НАЗВАТЬ ИНВАРИАНТ":""):screen==="game"&&stage==="observe"&&card&&draft.states.length<card.arabic?"НАЗВАТЬ СОСТОЯНИЕ":screen==="game"&&stage==="shape"?"ОТВЕТИТЬ СЛОЮ":screen==="game"&&stage==="act"?(!draft.induction.trim()?"НАЗВАТЬ ИНДУКЦИЮ":!draft.inversion.trim()?"НАЗВАТЬ ИНВЕРСИЮ":""):"";
 useEffect(()=>{const w=window as VoiceWindow;setVoiceReady("speechSynthesis" in window&&"SpeechSynthesisUtterance" in window);setRecognitionReady(Boolean(w.SpeechRecognition||w.webkitSpeechRecognition));return()=>window.speechSynthesis?.cancel()},[]);
 useEffect(()=>{if(!voiceEnabled||!voiceReady)return;const timer=window.setTimeout(()=>speak(narration),260);return()=>window.clearTimeout(timer)},[voiceEnabled,voiceReady,screen,turn,stage]);
 function speak(text=narration){if(!voiceReady)return;window.speechSynthesis.cancel();const utterance=new SpeechSynthesisUtterance(text);utterance.lang="ru-RU";utterance.rate=.92;utterance.pitch=.92;const russian=window.speechSynthesis.getVoices().find(v=>v.lang.toLowerCase().startsWith("ru"));if(russian)utterance.voice=russian;utterance.onstart=()=>setSpeaking(true);utterance.onend=()=>setSpeaking(false);utterance.onerror=()=>setSpeaking(false);window.speechSynthesis.speak(utterance)}
 function toggleVoice(){if(!voiceReady){setVoiceNotice("На этом устройстве голосовое воспроизведение недоступно.");return}if(voiceEnabled){window.speechSynthesis.cancel();setSpeaking(false);setVoiceEnabled(false)}else{setVoiceNotice("");setVoiceEnabled(true)}}
 function listenContext(){if(!dictationLabel)return;const w=window as VoiceWindow,Ctor=w.SpeechRecognition||w.webkitSpeechRecognition;if(!Ctor){setVoiceNotice("Распознавание речи недоступно. Используй диктовку клавиатуры — Скеллу продолжит вести цикл голосом.");return}window.speechSynthesis?.cancel();setSpeaking(false);const recognition=new Ctor();recognition.lang="ru-RU";recognition.interimResults=false;recognition.maxAlternatives=1;recognition.continuous=false;recognition.onresult=event=>{const text=event.results[0][0].transcript.trim();if(screen==="setup"&&!intent.trim())setIntent(text);else if(screen==="setup"&&!invariant.trim())setInvariant(text);else if(screen==="game"&&stage==="observe"&&card)setDraft(d=>d.states.length<card.arabic?{...d,states:[...d.states,text]}:d);else if(screen==="game"&&stage==="shape")setDraft(d=>({...d,reflection:text}));else if(screen==="game"&&stage==="act"&&!draft.induction.trim())setDraft(d=>({...d,induction:text}));else if(screen==="game"&&stage==="act")setDraft(d=>({...d,inversion:text}));setVoiceNotice("")};recognition.onerror=()=>setVoiceNotice("Не расслышала. Нажми ещё раз и произнеси одну короткую мысль.");recognition.onend=()=>setListening(false);setListening(true);setVoiceNotice("Слушаю…");recognition.start()}
 ...
 return <main className="appShell">
  ...
  <div className="skelluDock"><button className={`skelluOrb ${speaking?"speaking":""} ${listening?"listening":""}`} aria-label="Открыть голосового помощника Скеллу" onClick={()=>setSkelluOpen(v=>!v)}><i>◉</i><span>СКЕЛЛУ</span></button>{skelluOpen&&<aside className="skelluPanel" aria-live="polite"><header><div className="skelluAvatar"><i>◉</i></div><div><b>СКЕЛЛУ</b><span>{listening?"слушает":speaking?"говорит":voiceEnabled?"ведёт цикл":"голосовой проводник"}</span></div><button aria-label="Закрыть Скеллу" onClick={()=>setSkelluOpen(false)}>×</button></header><p>{narration}</p><div className="skelluActions"><button className={voiceEnabled?"active":""} onClick={toggleVoice}>{voiceEnabled?"ВЫКЛЮЧИТЬ ГОЛОС":"ВКЛЮЧИТЬ ВЕДЕНИЕ"}</button><button onClick={()=>speak()} disabled={!voiceReady}>↻ ПОВТОРИТЬ ШАГ</button></div>{dictationLabel&&<button className={`dictation ${listening?"active":""}`} onClick={listenContext} disabled={listening}>{listening?"● СЛУШАЮ…":`● ${dictationLabel}`}</button>}<small>{voiceNotice||(!recognitionReady?"Сопровождение доступно. Для ответа можно использовать диктовку клавиатуры.":"Говори коротко: одна мысль — одно нажатие.")}</small></aside>}</div>
